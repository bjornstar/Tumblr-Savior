...
cert_exists "private_key.key" "This RSA private key was generated by yourself (and should match your CSR)"
...
[ ! -e "${sigsizefile}" ] && openssl dgst -sign "${certdir}/private_key.key" -binary < "${certdir}/private_key.key" | wc -c > "${sigsizefile}"
...
openssl rsautl -sign -inkey "${certdir}/private_key.key" -in tmp-digest.dat -out tmp-sig.dat

xar --inject-sig tmp-sig.dat -f "${extzfile}"

rm -f tmp-sig.dat tmp-digest.dat "${certdir}/size.txt"
